...
# 2024/9/16
# Todo: 把吃的檔案轉成SWAG Dataset的格式，格式參考：https://huggingface.co/datasets/allenai/swag/viewer/regular/train?row=0
# 然後模型沒意外應該是用這個：https://huggingface.co/ckiplab/bert-base-chinese
# 或是這個：https://huggingface.co/google-bert/bert-base-chinese
...
def main():
    ...
    def addends(examples):
        ...
        for pid in examples["paragraphs"]:
            ...
            if pid == examples["relevant"]:
                examples["label"] = i
            i += 1
        return examples
    raw_datasets = raw_datasets.map(addends)
    ...
    # 這裡要改，目前是把sent1跟sent2都用問題來餵，希望沒問題...
